-- Hardening (surfaced by adversarial testing):
--   1. Validate that `target` is a real spirit for every rite kind that has one
--      (only 'newdeed' carries no target). Previously only the nominator was
--      checked, so a direct RPC caller could raise phantom offerings/deeds/etc.
--      against non-members, which then applied to nothing.
--   2. Lock the single grove_state row (SELECT ... FOR UPDATE) so concurrent
--      proposals/votes serialise instead of clobbering the shared jsonb blob.
...
begin
  -- lock the single state row so concurrent proposals/votes serialise instead of
  -- clobbering each other's writes to the shared jsonb blob
  ...
  -- the target must be a real spirit too, for every kind that has one
  -- (only 'newdeed' adds to the shared deed list and carries no target)
