-- Hardening: lock the single grove_state row (SELECT ... FOR UPDATE) so a flurry
-- of concurrent votes serialises instead of clobbering the shared jsonb blob.
-- Logic is otherwise identical to 20260701114852 (nominator rewards + offer parity).
...
begin
  -- lock the single state row so a flurry of concurrent votes can't clobber each
  -- other's writes to the shared jsonb blob
